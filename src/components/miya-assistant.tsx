  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.reply.trim() === '[IGNORE]') {
        ...
        const miyaMessage: Message = { text: response.reply, sender: 'miya', imageUrl: response.imageUrl };
                ...
                {messages.map((msg, index) => (
                  <div key={index} className={`flex flex-col gap-1 ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                    ...
                    >
                     ...
                     {msg.imageUrl && (
                        <div className="max-w-[80%] p-2 bg-muted rounded-lg">
                            <Image 
                                src={msg.imageUrl} 
                                alt="Generated by Miya" 
                                width={200} 
                                height={200}
